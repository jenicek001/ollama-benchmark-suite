...
*   **Context**: Default Ollama context unless noted. The 8845HS system used `num_ctx=8192` for `gemma4:31b`; the 8060S system below was validated at the full default `262144` tokens.
*   **Offloading**: All model layers offloaded to iGPU via ROCm unless noted

---

## System 1: AMD Ryzen AI Max 395 class system — Radeon 8060S / gfx1151

### Hardware
| Component | Detail |
| :--- | :--- |
| **CPU** | AMD Ryzen AI MAX+ PRO 395 |
| **GPU** | AMD Radeon 8060S / gfx1151, 40 CUs @ 2900 MHz |
...
| **Compute Stack** | ROCm user-space; `HSA_OVERRIDE_GFX_VERSION=11.0.0` on this host |
| **Inference Engine** | Ollama v0.20.4 |
| **GPU Memory (SMI)** | 96.0 GiB VRAM + 15.6 GiB GTT |
...
| **gemma4:31b** | 31B dense | Q4_K_M | **10.33** | 61/61 (100%) | 18.4 GiB weights + 23.5 GiB KV + 0.9 GiB graph = **44.0 GiB** | 262144 | Repo prompt, `num_predict=128`; default 256K context validated |
...
> Note: Earlier repo notes referred to the `gfx1151` iGPU as "Radeon 890M". Current ROCm/Ollama reporting on this machine identifies it as `Radeon 8060S`, so the setup and benchmark notes now use the live-reported name.
...
1.  **iGPU Capability**: Both systems can run models that typically require enterprise GPUs. The 8060S / gfx1151 system with 96 GB VRAM extends this to 120B+ parameter models.
2.  **Sweet Spot**: 8B–20B range offers the best speed/quality balance (20–40 t/s on the 8060S system, ~15–25 t/s expected on 780M).
...
5.  **gpt-oss:120b outlier**: Runs at 33 t/s on the 8060S system despite 120B params, suggesting it is a sparse/MoE model behaving like a much smaller active-parameter model.
